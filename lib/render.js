...
.home{display:flex;align-items:center;gap:8px;color:var(--text-secondary);font-size:13px;font-weight:600;transition:color var(--dur-fast) var(--ease-standard);}
.home:hover{color:var(--ink);}
.wrap{max-width:760px;margin:0 auto;padding:30px 22px 64px;}
.h1{font-family:var(--font-serif);font-size:22px;font-weight:600;margin:0 0 16px;letter-spacing:-.02em;}
pre{white-space:pre-wrap;word-wrap:break-word;font:14px/1.6 var(--font-mono);color:var(--ink);background:var(--surface);border:1px solid var(--hairline);border-radius:var(--radius-lg);box-shadow:var(--shadow-resting);padding:20px;}
@media (max-width:520px){.topbar{padding:0 16px;}.wrap{padding:24px 16px 48px;}}
</style></head>
<body>
  <div class="topbar"><a class="home" href="${base}/">${ICON.back}All prototypes</a></div>
  <div class="wrap"><h1 class="h1">How to ship a prototype</h1><pre>${body}</pre></div>
</body></html>`;
}
